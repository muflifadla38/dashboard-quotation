...
class Trend
{
    ...
    public function aggregate(string $column, string $aggregate, bool $isAggregate = true): Collection
    {
        $query = $isAggregate ? ", {$aggregate}({$column}) as aggregate" : "";

        $values = $this->builder
            ->toBase()
            ->selectRaw("{$this->getSqlDate()} as {$this->dateAlias}" . $query)
    ...
    public function sum(string $column, bool $isAggregate = true): Collection
    {
        return $this->aggregate($column, 'sum', $isAggregate);
    }
